...
class Trainer:
    ...
    def train_ordinary(self, 
                        ...
                        val_criterion=None, 
                        key=None):
        """ Train the NCF using Ordinary Alternating Minimisation: https://proceedings.mlr.press/v97/li19n.html """
        ...
        def train_step_node(node, contexts, batch, weights, opt_state, key):
            print('\nCompiling function "train_step" for neuralode ...')
        ...
        for epoch in range(nb_epochs):
            ...
            for i, batch in enumerate(self.dataloader):
                ...
                ## Opdate environment-agnostic parameters
                ...
                if update_context_every> 0 and i%update_context_every==0:
            ...
            if epoch%print_error_every==0 or epoch<=3 or epoch==nb_epochs-1:
                ...
                if epoch > 0:
                    self.learner.save_learner(save_path+"checkpoints/", suffix=epoch)
                ...
                if val_dataloader is not None:
                    ...
                    if ind_crit <= jnp.stack(val_losses)[:,1].min() and save_path:
                        print(f"        Saving best model so far ...", flush=True)
    ...
    def adapt_bulk(self, 
                    ...
                    save_path=False, 
                    key=None):
        """Adapt the model to new environments (in bulk) on the provided dataset. """
    ...
    def restore_adapted_trainer(self, path, data_loader=None):
        ...
        histories = np.load(path+"adapt_histories.npz")
