...
def get_new_key(key=None, num=1):
    """ Returns a new key for jax operations """
    if key is None:
        print("WARNING: No key provided for splitting, using time as seed")
...
def generate_new_keys(key=None, num=1):
    """ Returns a new key for jax operations. Useful when num > 1 """
    if key is None:
        print("WARNING: No key provided for splitting, using time as seed")
        key = jax.random.PRNGKey(time.time_ns())
...
def sbplot(*args, ax=None, figsize=(6,3.5), x_label=None, y_label=None, title=None, x_scale='linear', y_scale='linear', xlim=None, ylim=None, **kwargs):
    """ Wrapper function for matplotlib's (and seaborn) plot """
    if ax==None:
...
## Utility function to calculate losses
...
def l2_norm_traj(xs, xs_hat):
    total_loss = jnp.mean((xs - xs_hat)**2, axis=-1)
...
## Utility function to setup the run folder
...
def setup_run_folder(folder_path, script_name):
    """ Copy the run script, the module files, and create a folder for checkpoints and the adaptation. """
