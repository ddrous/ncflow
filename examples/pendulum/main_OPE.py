...
run_folder = "./18112024-123710-OnePerEnv-InD/"
...
if train == True:
    ...
    run_folder = "./runs/18112024-123710-OnePerEnv-InD/"
...
for adapt_env in range(0, 25, 6):      ## Nb of adaptation environments
...
times = ["2 mins 47 secs", "3 mins 57 secs", "4 mins 50 secs", "5 mins 23 secs", "6 mins 11 secs"]

## Gime me the mean the standard deviation of the times in mins
times_mins = [int(time.split(" ")[0]) for time in times]
times_secs = [int(time.split(" ")[2]) for time in times]

times_mins = [time_mins + time_secs/60 for time_mins, time_secs in zip(times_mins, times_secs)]

print(f"Mean time in mins: { np.mean(times_mins) :.2f}")
print(f"Std Dev time in mins: { np.std(times_mins) :.2f}")
