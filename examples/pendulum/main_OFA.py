...
run_folder = "./18112024-123710-OneForAll/"
...
## Adaptation hyperparameters (though there is no adaptation) ##
adapt_test = True
...
print(f"\n\nCurrently training for all environments at once (OFA) \n")
...
old_shape = train_dataloader.dataset.shape
train_dataset = train_dataloader.dataset.reshape((-1, old_shape[2], old_shape[3]))[None, ...]
...
old_shape = val_dataloader.dataset.shape
val_dataset = val_dataloader.dataset.reshape((-1, old_shape[2], old_shape[3]))[None, ...]
val_dataloader = DataLoader(dataset=val_dataset, t_eval=val_dataloader.t_eval, batch_size=-1, shuffle=False, key=seed)

##%%

print(f"Dataset quantities:, nb_envs {nb_envs}, nb_trajs_per_env {nb_trajs_per_env}, nb_steps_per_traj {nb_steps_per_traj}, data_size {data_size}")
...
if adapt_test:
    ...
    adapt_dataloader_test = DataLoader(adapt_folder+"adapt_test_data.npz", adaptation=False, data_id="170846", key=seed)
    old_shape = adapt_dataloader_test.dataset.shape
    adapt_dataset_test = adapt_dataloader_test.dataset.reshape((-1, old_shape[2], old_shape[3]))[None, ...]
    adapt_dataloader_test = DataLoader(dataset=adapt_dataset_test, t_eval=adapt_dataloader_test.t_eval, batch_size=-1, shuffle=False, key=seed)
