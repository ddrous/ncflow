...
run_folder = "./runs/28032024-195313/"
...
if train == True:
    ...
    for i, prop in enumerate(np.linspace(1.0, 1.0, 1)):
        ...
        trainer.train(nb_epochs=nb_epochs, print_error_every=print_error_every*(2**0), update_context_every=1, save_path=trainer_save_path, key=seed, val_dataloader=val_dataloader, int_prop=prop)
...
if finetune:
    ...
    opt_node = optax.adabelief(1e-4)
    opt_ctx = optax.adabelief(1e-4)
    ...
    trainer.train(nb_epochs=5000, print_error_every=100, update_context_every=1, save_path=finetunedir, key=seed)
