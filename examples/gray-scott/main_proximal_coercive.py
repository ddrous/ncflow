...
class Augmentation(eqx.Module):
    ...
    def __init__(self, data_res, kernel_size, nb_int_channels, context_size, key=None):
        ...
        self.layers_data = [lambda x: jnp.stack(vec_to_mats(x, data_res, 2), axis=0),
                            ...
                            circular_pad,
                            eqx.nn.Conv2d(2, cnt_chans, kernel_size, groups=1, key=keys[0]), self.activations[5]]

        # self.layers_shared = [
        #     # circular_pad, 
        #                       My2DConv(cnt_chans*2, nb_int_channels, kernel_size, groups=3, key=keys[6]), self.activations[1],
        #                     #   circular_pad, 
        ...
        self.layers_shared = [
                              circular_pad, 
                              eqx.nn.Conv2d(cnt_chans*2, cnt_chans, kernel_size, groups=1, key=keys[7]), self.activations[1],
                              circular_pad, 
                              eqx.nn.Conv2d(cnt_chans, cnt_chans, kernel_size, groups=1, key=keys[7]), self.activations[2],
                              circular_pad, 
                              eqx.nn.Conv2d(cnt_chans, 2, kernel_size, groups=1, key=keys[7]), self.activations[3],
                              lambda x: x.flatten()]
...
# for seed in range(8000, 6*10**3, 10):
    ...
    adapt_test_dataloader = DataLoader(adapt_folder+"adapt_data_test.npz", adaptation=True, batch_size=1, key=seed, data_id="082026")
